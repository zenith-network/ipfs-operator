...
impl Default for Metrics {
    fn default() -> Self {
        let reconcile_duration = HistogramVec::new(
            histogram_opts!(
                "ipfsnode_controller_reconcile_duration_seconds",
        ...
        let failures = IntCounterVec::new(
            opts!(
                "ipfsnode_controller_reconciliation_errors_total",
        ...
        let reconciliations = IntCounter::new(
            "ipfsnode_controller_reconciliations_total",
            "reconciliations",
        )
        .unwrap();
        Metrics {
...
impl Metrics {
    ...
    pub fn reconcile_failure(&self, ipfsnode: &IpfsNode, e: &Error) {
        self.failures
            .with_label_values(&[ipfsnode.name_any().as_ref(), e.metric_label().as_ref()])
            .inc()
    }
